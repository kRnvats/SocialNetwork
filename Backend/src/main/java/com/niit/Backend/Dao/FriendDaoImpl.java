...
private SessionFactory sessionFactory;
	...
	public List<r> pendingRequests(String toId) {
		Session session = sessionFactory.getCurrentSession();
		Query<r> query= session.createQuery("from Friend where toId=? and status='p'");
	...
	public List<String> listOfFriends(String userName) {
		...
		SQLQuery query2= session.createSQLQuery("select toId from Friend where fromId=? and status='A'")
				.addScalar("fromId",StandardBasicTypes.STRING);
